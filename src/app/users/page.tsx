...
//   SCOPE: Read, Add, Delete users and send tests; marks a user whose Steam profile stopped answering
//   DEPENDS: M-API, M-USERHEALTH
//   LINKS: M-UI
// END_MODULE_CONTRACT
...
import { UserPlus, Trash2, Send, Loader2, AlertTriangle } from "lucide-react";
import { USER_FAILURE_ALERT_THRESHOLD } from "../../lib/userHealth";
                            ...
                            <div key={u.id} className="p-4 flex items-center justify-between hover:bg-zinc-800/20 transition-colors">
                                <div>
                                    <h4 className="font-semibold text-zinc-100 flex items-center gap-2">
                                        {u.name}
                                        {u.consecutiveFailures >= USER_FAILURE_ALERT_THRESHOLD && (
                                            <span
                                                className="inline-flex shrink-0"
                                                title={u.lastError ? `Мониторинг не работает: ${u.lastError}` : "Мониторинг не работает"}
                                            >
...
//   LAST_CHANGE: [v1.1.0 - Show an AlertTriangle marker next to a user's name once their poll failures reach USER_FAILURE_ALERT_THRESHOLD, with the last error in the hover title]
//   LAST_CHANGE_2: [v1.2.0 - Import USER_FAILURE_ALERT_THRESHOLD from lib/userHealth.ts instead of a hand-duplicated local literal, so the marker and the core.ts alert can never drift apart]
// END_CHANGE_SUMMARY
