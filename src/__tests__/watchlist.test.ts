...
describe("M-WATCHLIST: scheduled discount checks", () => {
    ...
    it("records price history and sends an alert when a discount appears", async () => {
        ...
        expect(prismaMock.priceHistory.create).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({ discountPct: 50, discountPctKz: 30 }),
        }));
        ...
        expect(prismaMock.watchedGame.update).toHaveBeenCalledWith({
            where: { id: 1 },
            data: { lastDiscountPct: 50, lastDiscountPctKz: 30 },
        });
    });
