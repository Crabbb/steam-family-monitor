...
async function buildCardFromDetails(
    ...
    return {
        ...
        discountPctKz: detailsKz?.price_overview?.discount_percent ?? 0,
        // KZ
...
export function formatPriceCardHtml(card: PriceCardData): string {
    ...
    if (card.isFree) {
        ...
        if (card.priceKzFormatted && card.priceKzInRub !== null) {
            ...
            if (card.discountPctKz > 0) msg += ` <b>(-${card.discountPctKz}%)</b>`;
            msg += `\n`;
        }
...
export function formatWatchlistAlertHtml(
    ...
    triggeredRegions: ("RU" | "KZ")[] = [],
): string {
    const where = triggeredRegions.length > 0 ? ` (${triggeredRegions.join(" и ")})` : "";
    let msg = `🔔 <b>Скидка на отслеживаемую игру${where}!</b>\n\n`;
...
//   LAST_CHANGE_3: [v2.3.0 - Card carries the KZ discount percent alongside RU]
//   LAST_CHANGE_4: [v2.4.0 - Alert names the region where the discount appeared]
// END_CHANGE_SUMMARY
